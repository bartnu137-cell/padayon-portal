...
async function liveReloginFromRememberIfPossible() {
  // Attempts backend login using stored Remember Me creds (if available).
  // This avoids forcing an admin logout/login after changing backend URL.
  ...
  const u = String(saved?.username || '').trim();
  const p = String(saved?.password || '');

  if (!u || !p) return { ok: false, reason: 'no_saved_credentials' };
  if (u.toLowerCase() !== String(session.username || '').toLowerCase()) return { ok: false, reason: 'different_user' };

  const res = await liveTryBackendLogin(u, p);
  if (res?.ok) {
    // Backend becomes source-of-truth for role when auth succeeds
    if (res.user?.role) {
      session.role = res.user.role;
      // Update admin-only cards in Categories
    ...
    return { ok: true, reason: 'relogged', user: res.user };
  }

  return { ok: false, reason: res?.reason || 'failed', error: res?.error || null };
}
...
async function liveTryBackendLogin(username, password) {
  if (!liveIsEnabled()) return { ok: false, reason: 'no_backend' };
  try {
    ...
    if (json?.token) {
      ...
      return { ok: true, token: json.token, user: json.user || { username, role: 'user' } };
    }
...
async function verifyLogin() {
  ...
  // 1) Local (offline) accounts
  let acct = findAccount(userIn, passIn);

  // 2) Backend accounts (Render) — used for cross-device login + live sync
  // If backend login succeeds, it becomes the source of truth for role.
  if (liveIsEnabled()) {
    const res = await liveTryBackendLogin(userIn, passIn);
    if (res?.ok) {
      acct = {
  ...
  // Start live connection (online users + activity tracking + live updates)
  try {
    liveBackendUrl = getConfiguredBackendUrl();
    liveAuthToken = getLiveToken();
    updateLiveStatusUI();
  ...
  setTimeout(() => {
    ...
    // admin-only UI in category-level menus
    const adminCard = document.getElementById('admin-card');
    if (adminCard) adminCard.classList.toggle('hidden', session.role !== 'admin');
    ...
    // Both users and admins land on the landing page.
    // Admin can open the Admin Panel from the Category menu.
